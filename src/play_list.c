...
void m_create_play_list ( MPlayList*				obj_struct,
	...
	m_element_create ( e, name, obj_struct,
						1, 1, pos,
						&draw,
						0,
						0,
...
static void draw_item ( MPlayList *obj, MakiseGUI *gui, MPlayList_Item *pl_i, MakiseStyleTheme_PlayList *c_th, uint32_t x, uint32_t y, uint32_t w, uint32_t eh ) {
	...
	makise_d_string_frame(	gui->buffer,													// Draw time string.
...
static void header_text_draw ( const MPlayList* obj, MakiseGUI *gui, const int16_t* x, int16_t* y, const uint32_t* height_block, uint32_t* free_h ) {
...
static void drawing_scroll ( const MPlayList* obj, MakiseGUI *gui, uint32_t field_h, uint32_t field_y, const uint32_t y_pos, const uint32_t h_scroll ) {
...
MResult draw ( MElement* b, MakiseGUI *gui ) {
	...
	_m_e_helper_draw_box_param( gui->buffer, &b->position, st->border_c, st->bg_color, st->double_border );
	header_text_draw( obj, gui, &x, &y, &obj->eh, &h );					// Draw header.
	...
	for ( uint32_t i = 0; i < obj->item_array_len; i++ ) {
		...
		draw_item( obj, gui, &obj->item_array[ i ], st, x, y, w, obj->eh );
	...
	drawing_scroll( obj, gui, field_height, y_scroll, y_scroll + scroll_height * obj->selected->real_number_track + 1, scroll_height );
